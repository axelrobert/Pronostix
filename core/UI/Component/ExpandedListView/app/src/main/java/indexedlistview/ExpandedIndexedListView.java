...
import fr.freud.expandedlistview.R;
 ...
 */
public class ExpandedIndexedListView extends RelativeLayout {
    ...
    private static final int DEFAULT_SIDE_INDEX_BACKGROUND = R.drawable.default_side_index_border;
    ...
    public ExpandedIndexedListView(Context context) {
        this(context, null);
    }

    public ExpandedIndexedListView(Context context, AttributeSet attrs) {
        super(context, attrs);

        mContext = context;
        LayoutInflater inflater = (LayoutInflater) context
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        inflater.inflate(R.layout.expanded_indexed_list_view, this, true);
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.ExpandedIndexedListView, 0, 0);

        //Side index attributes
        float sideIndexWidth = a.getDimension(R.styleable.ExpandedIndexedListView_side_index_width,
                DEFAULT_SIDE_INDEX_WIDTH);
        mSideIndexBackground = a.getDrawable(R.styleable.
                ExpandedIndexedListView_side_index_background);
        if (mSideIndexBackground == null) {
            mSideIndexBackground = getResources().getDrawable(DEFAULT_SIDE_INDEX_BACKGROUND);
        }
        mSideIndexTextSize = a.getDimension(R.styleable.ExpandedIndexedListView_side_index_text_size,
                DEFAULT_SIDE_INDEX_TEXT_SIZE);
        mSideIndexTextColor = a.getColor(R.styleable.ExpandedIndexedListView_side_index_text_color,
                Color.parseColor(DEFAULT_SIDE_INDEX_TEXT_COLOR));

        //toast text attributes
        float toastTextTextSize = a.getDimension(R.styleable.ExpandedIndexedListView_toast_text_text_size,
                DEFAULT_TOAST_TEXT_SIZE);
        int toastTextColor = a.getColor(R.styleable.ExpandedIndexedListView_toast_text_color,
                Color.parseColor(DEFAULT_TOAST_TEXT_COLOR));
        a.recycle();

        //Instantiate views and layouts
        mList = (AnimatedExpandableListView) getChildAt(LIST_POSITION);
    ...
    /**
     * Get the list of the IndexedListView
     * @return : the list inside the indexedListView
     */
    public AnimatedExpandableListView getList() {
        return mList;
    }
